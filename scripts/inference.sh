#!/bin/bash    
#SBATCH --job-name=inference_clevr_rel
#SBATCH --output=logs/inference_sample_rel_%j.out
#SBATCH --error=logs/inference_sample_rel_%j.err
#SBATCH --nodes=1    
...
python scripts/inverse_relation_inference.py $MODEL_FLAGS $DIFFUSION_FLAGS --ckpt_path ./logs_clevr_rel_128/ema_0.9999_740000.pt